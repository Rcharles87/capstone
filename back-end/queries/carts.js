...
const getPreviousCarts = async (customer_id) => {
    try{
        const previousCarts = await db.any("SELECT products.id, products.name, order_details.carts_id, carts.id, carts.customer_id, carts.is_active FROM (products INNER JOIN order_details ON products.id=order_details.products_id) INNER JOIN carts ON order_details.carts_id=carts.id WHERE carts.is_active=false AND carts.customer_id=$1;", customer_id);
        console.log(previousCarts)
        return previousCarts;
        // inner join to combine product id name of product and carts id on one table  for all customers 
        // "SELECT products.id, products.name, order_details.carts_id FROM products INNER JOIN order_details on products.id=order_details.products_id";
